...
require("dashboard").setup({
	...
	config = {
		header = header,
		center = {
			{ icon = "  ", desc = "Find File", key = "f", action = "Pick files", shortcut = "C-p" },
			{ icon = "  ", desc = "Find Word", key = "w", action = "Pick grep_live", shortcut = "SPC fg" },
			{
				icon = "  ",
				desc = "Recent Files",
				key = "r",
				action = function()
					...
					MiniPick.start({ source = { items = vim.v.oldfiles } })
				end,
			},
			-- { icon = "  ", desc = "Recent Files", key = "r", action = "Pick oldfiles", shortcut = "" },
